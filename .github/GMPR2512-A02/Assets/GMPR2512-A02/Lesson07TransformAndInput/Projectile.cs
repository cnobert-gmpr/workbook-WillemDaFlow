...
namespace GMPR2512.Lesson07TransformAndInput
{
    public class Projectile : MonoBehaviour
    {
        ...
        {
            if (collider.gameObject.CompareTag("Aliens") && gameObject.CompareTag("AlienProjectile"))
            {
                return;
            }

            // 2. If a SHIP bullet hits an ALIEN, destroy the alien!
            if (collider.gameObject.CompareTag("Aliens") && gameObject.CompareTag("ShipProjectile"))
            {
            ...
            // 3. If an ALIEN bullet hits the SHIP, destroy the ship!
            if (collider.gameObject.CompareTag("Ship") && gameObject.CompareTag("AlienProjectile"))
            {
                Instantiate(_explosionTransform, collider.transform.position, transform.rotation);
                Destroy(collider.gameObject);
                Destroy(gameObject);
